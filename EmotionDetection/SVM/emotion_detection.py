...
def show_webcam_and_run(model, window_size=None, window_name='webcam', update_time=10):
    ...
    while read_value:
        for normalized_face, (x, y, w, h) in find_faces(web_cam_image):
            prediction = model.predict_proba(normalized_face)  # do prediction
            p = prediction[0]
            print(p)
...
if __name__ == '__main__':
    ...
    # clf.read('models/emotion_detection_model.xml')
    ...
    show_webcam_and_run(loaded_model, window_size=(1280, 720), window_name=window_name, update_time=8)
